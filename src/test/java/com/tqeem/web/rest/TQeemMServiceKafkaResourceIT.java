...
import com.tqeem.config.KafkaSseConsumer;
import com.tqeem.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class TQeemMServiceKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/t-qeem-m-service-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/t-qeem-m-service-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/t-qeem-m-service-kafka/unregister"));
                return;
            }
        }
